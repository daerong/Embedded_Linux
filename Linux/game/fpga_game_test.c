...
char chat_msg[] = "chat thread";
char num_baseball_msg[] = "num baseball thread";
...
void* num_baseball_func(void *data);
...
int main(int argc, char** argv) {
	...
	pthread_t chat_func_thread;
	int chat_func_thread_id;			// pthread ID
	pthread_t num_baseball_func_thread;
	int num_baseball_func_thread_id;			// pthread ID
	void *thread_result;				// pthread return

	num_baseball_func_thread_id = pthread_create(&num_baseball_func_thread, NULL, num_baseball_func, (void *)&num_baseball_msg);
	...
	while (1) {
		...
		case 0: 
			break;
		case 1: 
			pthread_join(chat_func_thread, (void *)&thread_result);
			set_small_image(&fvs, pfbdata, proc_display, ICON_START, ICON_1_Y_START, "icon1.bmp");
						...
						if (cur.x >= ICON_START && cur.x < ICON_END) {
							if (cur.y >= ICON_1_Y_START && cur.y < ICON_1_Y_START + ICON_WIDTH) {	// 채팅
								if (!chat_mode) {		// 켤수만 있음.
									...
									menu_copy(display, proc_display);
									menu_update(&fvs, pfbdata, display);
									chat_func_thread_id = pthread_create(&chat_func_thread, NULL, chat_func, (void *)&chat_msg);
									chat_mode = 1;
								}
							}
...
void* chat_func(void *data) {
	...
	unsigned char *text_lcd_buf = (unsigned char *)malloc(sizeof(unsigned char)*TEXT_LCD_MAX_BUF);
	int text_buf_index;
