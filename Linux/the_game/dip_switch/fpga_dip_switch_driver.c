...
// read()만 구현
//static ssize_t iom_dip_switch_write(struct file *file, const char *buf, size_t count, loff_t *f_pos);
...
struct file_operations iom_dip_switch_fops = {
	...
	//.write = iom_dip_switch_write,
	.read = iom_dip_switch_read,
	.release = iom_dip_switch_release
};

static int iom_dip_switch_open(struct inode *inode, struct file *file) {
	if (dip_switch_port_usage) {				// 정상 종료 시 0을 반환
		return -EBUSY;
	}
